...
/* Google Map API Start */
    ...
    function initialize() {
	 ...
	 var zoomControl =  map.addControl({
		  position: 'top_left',
		  content: 'Auto Zoom: <b>'+ (zoomEnabled == true ? "Enabled": "Disabled" + '</b>'),
		  style: {
			margin: '10px',
			padding: '9px 23px',
		  ...
		  events: {
			...
			//debugger;
				...
				zoomControl.innerHTML = 'Auto Zoom: <b>'+ (zoomEnabled == true ? "Enabled": "Disabled" + '</b>');
			}
		  }
		});
	...
	window.onkeyup = function(e) {
	  ...
	  //debugger;
	   ...
	   // google.maps.event.addListener(theMarker, 'rightclick', function(e) {
		//  debugger;
		//    if (this.runID !== undefined && this.runID != null) {
		//		angular.element(".columns").scope().removeJobFromMap(this.position.lat(), this.position.lng(), this.jn);
		//		return;
		//	 }
		 ...
		 map.setContextMenu({
			  control: 'marker',
			  options: [{
				title: 'Transfer Job to Another Run',
				name: 'transferJobToAnotherRun',
				action: function(e) {
					//this.setCenter(e.latLng.lat(), e.latLng.lng());
					 angular.element(".columns").scope().transferToAnotherRunFromMap(e.marker.jobID, e.marker.runID);
				}
			  }, {
				title: 'Set End',
				name: 'setJobEndFromMap',
				action: function(e) {
				  angular.element(".columns").scope().setJobEndFromMap(e.latLng.lat(), e.latLng.lng());
				}
			  },
			  {
				title: 'Remove From Run',
				name: 'removeJobFromMap',
				action: function(e) {
					//debugger;
				 if (e.marker.runID !== undefined && e.marker.runID != null) {
					if (confirm('Are you sure you want to remove this job from the locked run?')) {
						  // Remove from run
						 angular.element(".columns").scope().removeJobFromRun(e.marker.jobID);
						} else {
						  // Do nothing!
						}
				 }else{
					angular.element(".columns").scope().removeJobFromMap(e.marker.position.lat(),e.marker.position.lng(), e.marker.jobID);
				 }	
				}
			  }
			  ]
	  ...
	  // Ctrl + Left click = add into (locked)run
       google.maps.event.addListener(theMarker, 'click', function(e) {
	     //debugger;
		  if (this.runID !== undefined && this.runID != null) {
			angular.element(".columns").scope().addToRunFromMap(this.position.lat(), this.position.lng(), this.jn, this.jobID, this.runID);
		  } else {
	    ...
	    //google.maps.event.addListener(theMarker, 'rightclick', function(e) {
		 ...
		 // }
          ...
          options: [
		  {
            ...
            name: 'add_toRun',
            action: function(e) {
                angular.element(".columns").scope().addToRunFromMap(e.marker.position.lat(),e.marker.position.lng(), e.marker.jn, e.marker.jobID, e.marker.runID);
            }
          },
		   {
            title: 'Set End',
            name: 'setJobEndFromMap',
            action: function(e) {
              angular.element(".columns").scope().setJobEndFromMap(e.marker.position.lat(),e.marker.position.lng());
            }
          },
          {
            title: 'Remove From Run',
 ...
 function drawMarker(route, sorted){
        ...
        var multiRuns = angular.element(".columns").scope().multiSelectedRunsJobs;
            for (var i = 0; i < multiRuns.length; i++) {
                  //  var pinImage = new google.maps.MarkerImage("https://raw.githubusercontent.com/Concept211/Google-Maps-Markers/master/images/marker_" + multiRuns[i].color + (multiRuns[i].ro || '').toString() + ".png",
          //  new google.maps.Size(131, 154),
          //  new google.maps.Point(0, 0),
          //  new google.maps.Point(10, 34));

          var pinColor = multiRuns[i].color;
            var pinImage = new google.maps.MarkerImage("http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|" + pinColor,
              new google.maps.Size(131, 154),
              new google.maps.Point(0,0),
              new google.maps.Point(10, 34));
          var pinShadow = new google.maps.MarkerImage("http://chart.apis.google.com/chart?chst=d_map_pin_shadow",
            ...
            // var marker = new google.maps.Marker({
            // position: multiRuns[i],
            // map: map,
            // //icon: pinImage
            // icon: createPin(pinColor),
            // label: (multiRuns[i].ro || '').toString()
            // });
            // marker.jobID = multiRuns[i].jobID;
            ...
            map.addMarker({
					...
					// jobID for Transfer
					jobID: multiRuns[i].jobID,
					runID: multiRuns[i].runID
                  });
                }
          ...
          var marker =  map.addMarker({
				...
				// jobID for Transfer
				jobID: jobID,
				runID: angular.element(".columns").scope().defaultRunID
              });
	...
	if(zoomEnabled){
     ...
     angular.element(".columns").scope().runBuilderTotals();
 ...
 //                var multiRuns = angular.element(".columns").scope().multiSelectedRuns;
 //                for (var i = 0; i < multiRuns.length; i++) {
 //                    //  var pinImage = new google.maps.MarkerImage("https://raw.githubusercontent.com/Concept211/Google-Maps-Markers/master/images/marker_" + multiRuns[i].color + (multiRuns[i].ro || '').toString() + ".png",
 //                    //  new google.maps.Size(131, 154),
 //                    //  new google.maps.Point(0, 0),
 ...
 //    }
    ...
    function calcRoute(onlyDrawMarks) {
		...
		var runID = angular.element(".columns").scope().defaultRunID;
		...
		if(onlyDrawMarks)
		{
		  ...
		  checkboxArray = runBuilderJobs.map(x=> { return {lat:x.toLat, lng: x.toLng, jobID: x.BulkJobID, runID: runID}});
